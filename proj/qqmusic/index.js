...
function playBySongList(ipt){
	var xhr=new XMLHttpRequest();
	xhr.open("GET","proxy.php?songListId="+ipt,false);
	xhr.send();
	var xhrResp=xhr.responseText;
	var praiseJSON=JSON.parse(xhrResp);
	var data=praiseJSON;
	var songMidList=new Array();
	for(var i=0;i<data.cdlist[0].songlist.length;i++){
		songMidList.push(data.cdlist[0].songlist[i].mid);
	}
	player.play(songMidList);
	setTimeout("pageDataSync()", 500);
}
